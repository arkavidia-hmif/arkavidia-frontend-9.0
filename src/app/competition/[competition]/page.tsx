...
import { CompetitionTimeline, getCompetitionIdByName, getCompetitionTimelineWithCompetitionId, GetCompetitionTimelineWithCompetitionIdResponse } from '~/api/generated';
...
function CompetitionLanding({ params }: { params: Promise<{ competition: string }> }) {
  ...
  const [competitionMeta, setCompetitionMeta] = useState<{title: string, abbr: string, logoPath: string}>({title: '', abbr: '', logoPath: ''})
  const [competitionTimeline, setCompetitionTimeline] = useState<TimelineEventProps[]>( [{title: '', timeEnd: new Date()}])
  ...
  useEffect(() => {
    const fetchCompetitionData = async () => {
      try {
        ...
        const competitionResponse = await getCompetitionIdByName({ client: axiosInstance });
        ...
        if (matchedCompetition) {
          ...
          const competitionTimelineResponse = await getCompetitionTimelineWithCompetitionId({client: axiosInstance, path: {competitionId: matchedCompetition.id}})
          const competitionTimelineData: GetCompetitionTimelineWithCompetitionIdResponse = competitionTimelineResponse?.data
          const competitionTimelineTemp = transformToTimelineEvents(competitionTimelineData);
